...
function saveRemoteAppStateNow() {
  if (!appDataRemoteReady) {
    return;
  }
...
function persistAppState({ immediateRemote = false } = {}) {
...
async function deleteProfile(profileId) {
  ...
  render();
  persistAppState({ immediateRemote: true });
  showToast(`${profile.name}'s profile has been deleted.`);
}

function resetProfilePasscode(profileId) {
  if (activeUserLevel !== "admin") {
  ...
  showToast(`${profile.name}'s passcode has been reset.`);
}
...
profileForm.addEventListener("submit", (event) => {
  ...
  if (targetProfile) {
    Object.assign(targetProfile, profileData);
    activeProfileRole = profileData.role;
    showToast(`${profileData.name}'s profile has been updated.`);
  } else {
    freelancers.push({
      id: nextFreelancerId(),
      status: "available",
    ...
    activeProfileRole = profileData.role;
    showToast(`${profileData.name}'s profile has been created.`);
  }
