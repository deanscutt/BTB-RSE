...
function readableEmailColorValue(value = "") {
  const raw = String(value || "").trim();
  const withoutImportant = raw.replace(/\s*!important\s*$/i, "").trim();
  const compact = withoutImportant.replace(/\s+/g, "").toLowerCase();
  const invisibleValues = new Set(["transparent", "white", "#fff", "#ffff", "#ffffff", "#ffffffff"]);

  if (!withoutImportant || invisibleValues.has(compact)) {
    return "#1f2933";
  }

  const commaRgb = withoutImportant.match(/^rgba?\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})(?:\s*,\s*([\d.]+))?\s*\)$/i);
  const spaceRgb = withoutImportant.match(/^rgba?\(\s*(\d{1,3})\s+(\d{1,3})\s+(\d{1,3})(?:\s*\/\s*([\d.]+%?))?\s*\)$/i);
  const match = commaRgb || spaceRgb;

  if (match) {
    const [, red, green, blue, alphaValue] = match;
    const alpha = alphaValue?.endsWith("%") ? Number.parseFloat(alphaValue) / 100 : Number.parseFloat(alphaValue ?? "1");
    const isTransparent = Number.isFinite(alpha) && alpha <= 0.05;
    const isNearWhite = [red, green, blue].every((channel) => Number(channel) >= 245);
...
function sanitizeEmailStyle(value = "") {
  const allowedProperties = new Set(["color", "font-weight", "font-style", "text-decoration"]);
  ...
  return String(value)
    ...
    .map((rule) => {
      ...
      if (name === "color") {
        return `${name}: ${readableEmailColorValue(styleValue)}`;
      }
